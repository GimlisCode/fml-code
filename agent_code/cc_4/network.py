...
class QNetwork(pl.LightningModule):
    def __init__(self, features_out: int, gamma: float = 0.9, learning_rate: Optional[float] = 0.0001):
        ...
        self.conv_layers = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),  # 18x18
            torch.nn.ReLU(),
            ...
            torch.nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),  # 9x9
            torch.nn.ReLU(),
            torch.nn.BatchNorm2d(64),
        )

        self.linear_layers = torch.nn.Sequential(
            torch.nn.Linear(64 * 9 * 9, 264),
            torch.nn.ReLU(),
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        return self.linear_layers(self.conv_layers.forward(x).view((-1, 64 * 9 * 9)))
