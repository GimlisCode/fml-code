...
class QNetwork(pl.LightningModule):
    def __init__(self, features_out: int, gamma: float = 0.5, learning_rate: Optional[float] = 0.0001):
        ...
        self.conv_layers = torch.nn.Sequential(
            ...
            # torch.nn.ReLU(),
            # torch.nn.BatchNorm2d(8),
            torch.nn.Conv2d(in_channels=8, out_channels=8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),  # 9x9
            torch.nn.ReLU(),
            # torch.nn.BatchNorm2d(8),
            torch.nn.Conv2d(in_channels=8, out_channels=8, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),  # 9x9
            torch.nn.ReLU(),
            # torch.nn.BatchNorm2d(8),
        )

        self.linear_layers = torch.nn.Sequential(
            torch.nn.Linear(8 * 9 * 9, 128),
            torch.nn.ReLU(),
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        return self.linear_layers.forward(self.conv_layers.forward(x).view((-1, 8 * 9 * 9)))
