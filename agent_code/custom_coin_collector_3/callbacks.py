...
def setup(self):
    ...
    try:
        with open("train_data.json", "r") as file:
            self.train_data = json.loads(file.read())
    except (EOFError, FileNotFoundError):
        self.train_data = dict()
        self.train_data["reward"] = list()
        self.train_data["state_features_t"] = list()
        self.train_data["state_features_{t+1}"] = list()
        self.train_data["action"] = list()
